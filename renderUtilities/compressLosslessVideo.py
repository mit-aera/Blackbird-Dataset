...
def compressLosslessVideo(input_folder, file_extension, output_folder=None):
    ...
    # Calculate the FPS
    fps = len(
        files)/(getTimestampFromString(files[-1]) - getTimestampFromString(files[0]))
    ...
    inputSources = " -thread_queue_size 512 -framerate " + \
        str(fps)+" -pattern_type glob -i '" + "*" + file_extension + "' "
    ...
    # Create docker run
    docker_args = "docker run --rm -it --gpus 'device=0' --volume {}:/workspace willprice/nvidia-ffmpeg -y ".format(
        input_folder)
    ...
    command = docker_args + inputSources + cameraFilter + \
        encoderSettings + outputRateSettings + "lossless.mov"
    ...
    if (output_folder != input_folder):
        shutil.move(os.path.join(input_folder, "lossless.mov"),
                    os.path.join(output_folder, "lossless.mov"))
...
# For compressing tarball
        ...
        command = "find " + input_folder + " -name '*"+file_extension+"' -printf '%P\n' | parallel --bar gm convert " + \
            os.path.join(input_folder, "{}") + " " + os.path.join(input_folder, "{.}.png") + \
            " && find " + input_folder + " -name '*"+file_extension+"' -delete"
        print(command)
        ...
        command = "tar -cf " + \
            os.path.join(output_folder, "lossless.tar") + \
            " -C "+input_folder+" ."
        print(command)
        process = subprocess.Popen(command, shell=True)
